...
const APPS_SCRIPT_URL = ""; // Paste Apps Script URL vào đây nếu có
...
const fmtK = n => Math.abs(n)>=1e6 ? (n/1e6).toFixed(2)+'M' : Math.abs(n)>=1e3 ? (n/1e3).toFixed(0)+'K' : fmt(n);
...
const CD = {
  ...
  plugins:{
    ...
    tooltip:{backgroundColor:'#1a2235',borderColor:'rgba(255,255,255,0.08)',borderWidth:1,titleColor:'#e8edf5',bodyColor:'#94a3b8',padding:8}
  },
  scales:{
...
// ── NAV ─────────────────────────────────────
const PAGE_TITLES = {'p-chamcong':'📋 Chấm Công Hàng Ngày','p-sanluong':'🌿 Sản Lượng Tháng 5','p-luong':'💰 Hiệu Suất & Tiền Lương'};
document.querySelectorAll('.nav-item').forEach(btn=>{
  btn.addEventListener('click',()=>{
    ...
    if(pid==='p-chamcong') renderChamCong();
...
async function loadAllData() {
  ...
  try {
    // Fetch 3 sheets song song
    const [cc, hs, tl] = await Promise.all([
      fetchCSV('CHAM CONG'),
      fetchCSV('3. Hi\u1ec7u su\u1ea5t'),
      fetchCSV('4. Ti\u1ec1n l\u01b0\u01a1ng')
    ]);
    parseChamCong(cc);
    parseHieuSuat(hs);
    parseTienLuong(tl);
    dot.className='status-dot ok';
    stxt.textContent='Live · Google Sheets ✓';
  } catch(e) {
    console.warn('Sheets fetch failed, dùng fallback:', e);
    STATE.daily=FALLBACK_DAILY;
    STATE.workers=FALLBACK_WORKERS_ATT;
    STATE.hs_total=FALLBACK_HS_TOTAL;
    STATE.hs_workers=FALLBACK_HS_WORKERS;
    STATE.luong=FALLBACK_LUONG_WORKERS;
    dot.className='status-dot error';
    stxt.textContent='Offline · Dữ liệu Excel';
    document.getElementById('error-banner').style.display='flex';
    document.getElementById('error-msg').textContent='⚠ Không kết nối được Google Sheets — đang dùng dữ liệu từ file Excel. Sheet cần được Publish (File → Share → Publish to web).';
  }
...
function parseChamCong(rows) {
  ...
  const dayMap = {};
  valid.forEach(r => {
    const rawDate = r['NGAY'] || '';           // "01/05"
    const day = parseDay(rawDate);
    if (!day) return;
    const dateKey = `2026-05-${String(day).padStart(2,'0')}`;
    if (!dayMap[dateKey]) dayMap[dateKey] = {date:dateKey, day, n:0, ct:0, db:0, mb:0, mu_tuoi:0, mu_day:0};
    dayMap[dateKey].n++;
    if (r['Chính thức']==='TRUE') dayMap[dateKey].ct++;
    if (r['Dự bị']==='TRUE')      dayMap[dateKey].db++;
    if (r['Mưa bão']==='TRUE')    dayMap[dateKey].mb++;
    dayMap[dateKey].mu_tuoi += parseVN(r['Mủ tươi']);
    dayMap[dateKey].mu_day  += parseVN(r['Mủ dây']);
  });
  ...
  valid.forEach(r => {
    ...
    wMap[name].mu_tuoi += parseVN(r['Mủ tươi']);
    wMap[name].mu_day  += parseVN(r['Mủ dây']);
  });
...
function parseHieuSuat(rows) {
  ...
  STATE.hs_workers = rows
    .filter(r => {
      const name = r['Họ tên công nhân']||'';
      return name && name!=='HO TEN CONG NHAN' && !name.includes('HO TEN') && name.trim()!=='';
    })
    .map(r => ({
      ten: r['Họ tên công nhân'],
      kh:  parseVN(r['Kế hoạch sản lượng mủ tươi']),
      tt:  parseVN(r[' Sản lượng mủ tươi thực tế']),
      hs:  parseVN(r['Hiệu suất thực']) / 100,  // "106,93%" → 1.0693
...
function parseTienLuong(rows) {
  STATE.luong = rows
    .filter(r => {
      const name = r['Họ tên CN']||'';
      return name && name.trim()!=='' && name!=='Họ tên CN';
    })
    .map(r => ({
      ten:  r['Họ tên CN'],
      cong: parseVN(r['Tổng số công']),
      tcc:  parseVN(r['Tiền chuyên\ncần (Riel)']),
      mu_tuoi: parseVN(r['Tổng mủ\ntươi (kg)']),
      tsl:  parseVN(r['Tiền sản\nlượng (Riel)']),
      rank: r['Xếp hạng']||'',
      hs:   parseVN(r['Hiệu suất']) / 100,
      tong: parseVN(r['TỔNG LƯƠNG\n(Riel)'])
    }))
...
function renderAll(){renderChamCong();renderSanLuong();renderLuong();updateTopbar();}
...
function renderChamCong(){
  ...
  const wkFilter = d => {
    if(wk==='1') return d.day>=1&&d.day<=7;
    if(wk==='2') return d.day>=8&&d.day<=15;
    if(wk==='3') return d.day>=16&&d.day<=22;
    if(wk==='4') return d.day>=23&&d.day<=31;
    return true;
  };
  let daily=STATE.daily.filter(wkFilter);
  ...
  CHARTS['daily-att']=new Chart(document.getElementById('c-daily-att').getContext('2d'),{
    ...
    data:{labels:daily.map(d=>d.day+'/5'),datasets:[{label:'Số CN',data:daily.map(d=>d.n),
      backgroundColor:daily.map(d=>d.n>=27?'rgba(34,197,94,0.7)':d.n>=24?'rgba(234,179,8,0.7)':'rgba(239,68,68,0.65)'),
      borderRadius:4,barPercentage:0.7}]},
    options:{...CD,scales:{...CD.scales,y:{...CD.scales.y,min:18,max:32}}}
  });

  destroyChart('att-pie');
  const totCT=STATE.workers.reduce((s,w)=>s+w.ct,0);
  const totDB=STATE.workers.reduce((s,w)=>s+w.db,0);
  const totMB=STATE.workers.reduce((s,w)=>s+w.mb,0);
  ...
  document.getElementById('tb-att').innerHTML=sorted.map((w,i)=>{
    ...
    return `<tr><td>${i+1}</td><td><b>${w.ten}</b></td><td class="r">${w.n}</td><td class="r" style="color:var(--green);font-weight:700">${w.ct}</td><td class="r" style="color:var(--blue)">${w.db}</td><td class="r" style="color:var(--yellow)">${w.mb}</td><td class="c">${cc}</td><td class="r" style="color:var(--text2)">${fmt(w.mu_tuoi)}</td></tr>`;
  }).join('');
}
...
function renderSanLuong(){
  ...
  const totTT=hs.reduce((s,w)=>s+w.tt,0);
  const totKH=hs.reduce((s,w)=>s+w.kh,0);
  const hieuSuat=totKH?totTT/totKH:0;
  const diff=totTT-totKH;
  ...
  document.getElementById('k2-tt-pct').textContent=(hieuSuat*100).toFixed(1)+'% kế hoạch';
  ...
  document.getElementById('prog-pct').textContent=(hieuSuat*100).toFixed(1)+'%';
  ...
  document.getElementById('tb-sl').innerHTML=[...hs].sort((a,b)=>b.tt-a.tt).map((w,i)=>{
    const hsPct=(w.hs*100).toFixed(1), barW=Math.min(w.hs*100,130)/1.3;
    const barCls=w.hs>=1?'bar-green':w.hs>=0.85?'bar-orange':'bar-red';
    const status=w.hs>=1?'<span class="chip chip-green">✓ Đạt KH</span>':w.hs>=0.85?'<span class="chip chip-yellow">~ Gần đạt</span>':'<span class="chip chip-red">✗ Thiếu</span>';
...
function renderLuong(){
  ...
  document.getElementById('k3-total').textContent=fmtK(totLuong)+' R';
  document.getElementById('k3-avg').textContent=fmtK(totLuong/Math.max(1,all.length))+' R';
  ...
  CHARTS['luong-pie']=new Chart(document.getElementById('c-luong-pie').getContext('2d'),{
    ...
    options:{responsive:true,maintainAspectRatio:true,cutout:'60%',
      plugins:{legend:{display:true,position:'bottom',labels:{color:'#94a3b8',font:{size:10},padding:8}},
        tooltip:{...CD.plugins.tooltip,callbacks:{label:c=>`${c.label}: ${fmtK(c.raw)} R (${((c.raw/totLuong)*100).toFixed(1)}%)`}}}}
  });
  ...
  document.getElementById('tb-luong').innerHTML=data.map((d,i)=>{
    const hsPct=(d.hs*100).toFixed(1), barW=Math.min(d.hs*100,130)/1.3;
    ...
    return `<tr><td>${i+1}</td><td><b>${d.ten}</b></td><td class="r">${d.cong}</td><td class="r" style="color:var(--green)">${fmt(d.tcc)}</td><td class="r">${fmt(d.mu_tuoi)} kg</td><td class="r" style="color:var(--blue)">${fmt(d.tsl)}</td><td class="c"><span class="rank ${rankCls}">${d.rank||'B'}</span></td><td><div class="bar-inline"><div class="bar-track"><div class="bar-fill ${barCls}" style="width:${barW.toFixed(0)}%"></div></div><span class="bar-label" style="color:${clr(d.hs)}">${hsPct}%</span></div></td><td class="r" style="color:var(--orange);font-weight:800;font-family:var(--mono)">${fmt(d.tong)}</td></tr>`;
  }).join('');
